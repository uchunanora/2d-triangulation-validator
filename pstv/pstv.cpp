...
#pragma region GetData
  ...
  void get_xy_sort()
  {
    ...
    sort(x_sorted_vert.begin(), x_sorted_vert.end(), [](const vector<double> &alpha, const vector<double> &beta)
         { return alpha[0] < beta[0]; });
    sort(y_sorted_vert.begin(), y_sorted_vert.end(), [](const vector<double> &alpha, const vector<double> &beta)
...
#pragma region Tool
  ...
  {
    cout << "triangle1: ";
    disp_v(tri[tri_index1]);
    cout << "triangle2: ";
    disp_v(tri[tri_index2]);
...
#pragma omp parallel for
  ...
  */
  ...
  inline int intersection(vector<double> vert_a, vector<double> vert_b, vector<double> vert_c, vector<double> vert_d)
  {
    ...
    if (o1 * o2 < 0 && o3 * o4 < 0)
      return -1;
    if (o1 == 0 && onSegment(vert_a, vert_b, vert_c))
      return -1;
    if (o2 == 0 && onSegment(vert_a, vert_b, vert_d))
      return -1;
    if (o3 == 0 && onSegment(vert_c, vert_d, vert_a))
      return -1;
    if (o4 == 0 && onSegment(vert_c, vert_d, vert_b))
      return -1;
    return 1;
  }
  ...
  int check_convex_hull(vector<int> polygon)
  {
    ...
    if (unprocessed_set.size() == 0)
    {
      ...
      else
      {
        cout << "unprocessed_set: " << unprocessed_set.size() << endl;
        cout << "boundary_for_check size: " << boundary_for_check.size() << endl;
        cout << "boundary_for_check: ";
        disp_v(boundary_for_check);
        cout << "polygon size: " << polygon.size() << endl;
        cout << "polygon: ";
        disp_v(polygon);
        return -1; // Division Error
  ...
  vector<int> get_orientation_candidates(vector<int> polygon, vector<double> p1, vector<double> p2, int index_a)
  {
    ...
    for (int i = 0; i < PS - 3; ++i)
    {
      ...
      if (x1 <= x_min && x2 <= x_min)
        continue;
      if (x_max <= x1 && x_max <= x2)
        continue;
      if (y1 <= y_min && y2 <= y_min)
        continue;
      if (y_max <= y1 && y_max <= y2)
        continue;
...
{
  using pstv::boundary;
  using pstv::boundary_edges;
  using pstv::edge_map;
  using pstv::edges;
  using pstv::hyper_end_polygon_size;
  using pstv::hyper_polygon_size;
  using pstv::intervalTreeX;
  using pstv::intervalTreeY;
  using pstv::tree_flag;
  using pstv::tri;
  using pstv::triangle_map;
  using pstv::unprocessed_set;
  using pstv::vert;
  using pstv::vertex_map;
  using pstv::x_sorted_vert;
  using pstv::y_sorted_vert;
